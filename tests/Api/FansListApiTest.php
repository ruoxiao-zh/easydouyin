...
/**
 * FansListApiTest Class Doc Comment
 *
 * @category Class
 * @author   抖音小开
 */
class FansListApiTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Test case for fansListGet
     *
     * 粉丝列表.
     *
     */
    public function testFansListGet()
    {
...
$cursor = 0; // int | 分页游标, 第一页请求cursor是0, response中会返回下一页请求用到的cursor, 同时response还会返回has_more来表明是否有更多的数据。
        ...
        try {
        $result = $this->apiInstance->fansListGet($open_id, $access_token, $count, $cursor);
            print_r($result);
        } catch (Exception $e) {
