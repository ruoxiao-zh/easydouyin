...
/**
 * FollowingListApiTest Class Doc Comment
 *
 * @category Class
 * @author   抖音小开
 */
class FollowingListApiTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Test case for followingListGet
     *
     * 获取关注列表.
     *
     */
    public function testFollowingListGet()
    {
...
$cursor = 0; // int | 分页游标, 第一页请求cursor是0, response中会返回下一页请求用到的cursor, 同时response还会返回has_more来表明是否有更多的数据。
        ...
        try {
        $result = $this->apiInstance->followingListGet($open_id, $access_token, $count, $cursor);
            print_r($result);
        } catch (Exception $e) {
